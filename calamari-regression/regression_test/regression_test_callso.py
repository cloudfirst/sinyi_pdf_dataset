...
from sinobotocr import cv2_helper
from sinobotocr import cv2_helper2
from sinobotocr import my_pdf2img
from sinobotocr import my_log
from sinobotocr import find_ztgz
compare_ztgz = find_ztgz.gen_compare_ztgz()
...
def rec_img(path):
    ...
    dest_file = my_pdf2img.getImageFromPDF(path)
    ...
    orig, canny = cv2_helper.step_1_pre_processing_image(dest_file)
    
    # Step 2: 定位并提取表格部分
    logger.error("step_2_location_table ...")
    table       = cv2_helper.step_2_location_table(orig, canny)
    cv2_helper.save_processed_image(table,   "/tmp/table_"  + filename + ".png")
    
    # Step 3: 提取表格中每一行的文本
    logger.error("step_3_find_text_lines ...")
    text_blocks = cv2_helper2.step_3_find_text_lines_v2(table, filename)
    
    # Step 4: 识别表格中每一行的文本，并查找关键字极其取值
    logger.error("step_4_read_keyword_and_value ...")
    areas, ztgz, confidence = cv2_helper2.step_4_read_keyword_and_value_v2(text_blocks, filename)
